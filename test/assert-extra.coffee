...
assert.hasXmlTag = (xml, xpath, expected, message) ->
	...
	unless expected is undefined
		if not (expected is element[0].text())
			assert.fail xml, expected, message || "expected #{xml} to have xpath #{xpath} with content #{expected}", "hasTag", assert.hasXmlTag
...
assert.hasXmlTagMatch = (xml, xpath, expected, message) ->
	doc = libxmljs.parseXmlString xml
	element = doc.find(xpath)
	if element[0] is undefined
		throw new Error "XPath '#{xpath}' was not found in the document."
	else if not (expected.test(element[0].text()))
		assert.fail xml, expected, message || "expected #{xml} to have xpath #{xpath} with content #{expected}", "hasXmlTagMatch", assert.hasTag
